...
detail::alpha_spectrum::calibration_telemetry& detail::alpha_spectrum::shared_calibration_telemetry()
{
    static calibration_telemetry telemetry;
    return telemetry;
}
...
void alpha_spectrum::filter(const double* input, double* output)
{
    ...
    // Stage 4: publish telemetry for the settings panel.
    // If MTM is disabled, objective falls back to innovation objective.
    telemetry.rot_objective.store(mtm_enabled ? rot_mode_e : rot_objective_avg, std::memory_order_relaxed);
    telemetry.pos_objective.store(mtm_enabled ? pos_mode_e : pos_objective_avg, std::memory_order_relaxed);
    telemetry.rot_jitter.store(rot_jitter_sum / 3.0, std::memory_order_relaxed);
    telemetry.pos_jitter.store(pos_jitter_sum / 3.0, std::memory_order_relaxed);
    telemetry.rot_brownian_raw.store(rot_brownian_raw_avg, std::memory_order_relaxed);
    ...
    telemetry.rot_mode_peak.store(rot_mode_peak, std::memory_order_relaxed);
    telemetry.pos_mode_peak.store(pos_mode_peak, std::memory_order_relaxed);
    telemetry.ngc_coupling_residual.store(coupling_residual, std::memory_order_relaxed);
    telemetry.rot_alpha_min.store(*s.rot_alpha_min, std::memory_order_relaxed);
    telemetry.rot_alpha_max.store(*s.rot_alpha_max, std::memory_order_relaxed);
    telemetry.rot_curve.store(*s.rot_curve, std::memory_order_relaxed);
    telemetry.rot_deadzone.store(*s.rot_deadzone, std::memory_order_relaxed);
    telemetry.pos_alpha_min.store(*s.pos_alpha_min, std::memory_order_relaxed);
    telemetry.pos_alpha_max.store(*s.pos_alpha_max, std::memory_order_relaxed);
    telemetry.pos_curve.store(*s.pos_curve, std::memory_order_relaxed);
    telemetry.pos_deadzone.store(*s.pos_deadzone, std::memory_order_relaxed);
}
